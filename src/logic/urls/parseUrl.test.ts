...
describe('parseUrl', () => {
  it('should correctly parse url - origin, query-params, no hash', () => {
    const { origin, path, queryParams, hash } = parseUrl(
      'https://example.com/get-quotes/for/junk-removal?postal=12345',
    );
  ...
  it('should correctly parse url - no origin, query-params, no hash', () => {
    const { origin, path, queryParams, hash } = parseUrl(
      '/get-quotes/for/junk-removal?postal=12345',
    );
  ...
  it('should correctly parse url - origin, query-params, hash', () => {
    const { origin, path, queryParams, hash } = parseUrl(
      'https://example.com/get-quotes/for/junk-removal?postal=12345#open-menu',
    );
    expect(origin).toEqual('https://example.com');
    expect(path).toEqual('/get-quotes/for/junk-removal');
    expect(queryParams).toEqual({ postal: '12345' });
  ...
  it('should correctly parse url - origin, no query-params, hash', () => {
    const { origin, path, queryParams, hash } = parseUrl(
      'https://example.com/get-quotes/for/junk-removal#open-menu',
    );
    expect(origin).toEqual('https://example.com');
    expect(path).toEqual('/get-quotes/for/junk-removal');
  ...
  it('should correctly parse url - even if origin has a port defined', () => {
    const { origin, path, queryParams, hash } = parseUrl(
      'https://localhost.example.com:3443/get-quotes/for/junk-removal#open-menu',
    );
